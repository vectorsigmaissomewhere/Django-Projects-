...
## Example with code of Validators
admin.py
```python
from django.contrib import admin
from .models import Student
...
# creating a validation for roll
class StudentSerializer(serializers.ModelSerializer):
    # Adding the validator to the name field
...
Conclusion of Validators
```text
Check the serializers.py code only
This example validates the person with username r only
```
